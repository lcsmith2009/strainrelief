const APP_VERSION="v46-real-fixes";
...
/* ======================================================
   ...
   ====================================================== */
(function(){
  const VERSION = "v46-real-dom-patch";
  ...
  const RANDOM_POOL = [
    ...
    "Permanent Marker","Banana OG","Animal Cookies","Afghan Kush","Maui Wowie","Do-Si-Dos",
    "Ice Cream Cake","Cherry Pie","Tangie","White Widow","Papaya","Bruce Banner","MAC",
    "Oreoz","Sunset Sherbet","Slurricane","Bubba Kush","Green Crack","Sour Diesel",
    "Northern Lights","Cannatonic","ACDC","Zkittlez","Rainbow Belts","Forbidden Fruit",
    "Trainwreck","Skywalker OG","Blueberry","Lemon Tree","Blue Cheese","Platinum Cookies",
    "Georgia Pie","Pennywise","Purple Urkle","Mimosa","Sherbet","GMO Cookies","Kush Mints",
    "Tropicana Cookies","LA Confidential","Chemdawg","Headband","Strawberry Cough",
    "Lemon Haze","Super Silver Haze","Durban Poison","OG Kush","Forbidden Zkittlez"
  ];
  ...
  function chip(name){
    ...
    b.type="button";
    b.className="tag strain-pill v46-terpene-chip";
    b.textContent=name;
    return b;
  }

  function fixHero(){
    const h = [...document.querySelectorAll("#home h1,.hero h1,.home-hero h1")]
      .find(x => /find\s+your\s+wellness/i.test(x.textContent || ""));
    if(h){
    ...
    [...document.querySelectorAll("#home p,#home .hero p,#home [class*='hero'] p")].forEach(p=>{
  ...
  function removeJournalPills(){
    const saved=document.querySelector("#saved");
    if(!saved) return;

    const selectors=[
      ".journal-feelings",".feelings-row",".quick-feelings",".mood-pills",
      ".chip-row",".option-pills",".feeling-options",".mood-options",
      "[data-journal-pills]","[data-feeling-pills]"
    ];

    selectors.forEach(sel=>{
      saved.querySelectorAll(sel).forEach(el=>el.remove());
    });

    // Remove any row between the "How did it feel?" select and notes textarea if it is mostly buttons.
    const journal=[...saved.querySelectorAll(".wellness-journal,.journal-card,.journal-form,[class*='journal']")]
      .find(el => el.querySelector("select") && el.querySelector("textarea"));
    if(journal){
      [...journal.children].forEach(child=>{
        const buttons=child.querySelectorAll ? child.querySelectorAll("button") : [];
        const text=(child.textContent||"").toLowerCase();
        const looksLikeMood = /calm|relaxed|sleepy|focused|creative|hungry|clear-headed|too intense|not noticeable/.test(text);
        if(buttons.length >= 3 && looksLikeMood) child.remove();
      });
    }
  }

  function findTerpeneCards(){
    const terpNames=/myrcene|limonene|pinene|linalool|caryophyllene|terpinolene|humulene|ocimene/i;
    const roots=[
      document.querySelector("#terpeneExplorer"),
    ...
    const cards=[];
    roots.forEach(root=>{
      root.querySelectorAll(".terpene-card, article, .card, .glass-card, div").forEach(el=>{
        const txt=(el.textContent||"").trim();
        if(txt.length>60 && terpNames.test(txt)){
          // Choose card-sized elements, not whole learn page
          const rect=el.getBoundingClientRect();
          if(rect.height>80 && rect.height<900) cards.push(el);
        }
      });
    });

    // Deduplicate and prefer smaller deepest cards
    return [...new Set(cards)].filter(card=>{
      return ![...cards].some(other => other !== card && card.contains(other) && (other.textContent||"").match(terpNames));
    });
  }

  function randomizeTerpeneCards(){
    const cards=findTerpeneCards();
    cards.forEach((card,idx)=>{
      let row = card.querySelector("[data-terpene-examples],.example-strains,.strain-tags,.tags,.similar-scroll");
      if(!row){
        row=document.createElement("div");
        row.className="tags v46-terpene-examples";
      ...
      chosen.forEach(name=>row.appendChild(chip(name)));
    });
  }
  ...
  if("serviceWorker" in navigator){
    navigator.serviceWorker.getRegistrations?.().then(regs=>{
      regs.forEach(r=>r.update && r.update());
    }).catch(()=>{});
  }
