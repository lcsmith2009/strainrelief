const APP_VERSION="strainrelief-v44-real-journal-terpene-random";
...
/* ===== V43 TRUE RANDOM TERPENE EXAMPLES ===== */
(function(){
  ...
  function fixHero(){
    const hero = Array.from(document.querySelectorAll("#home h1,.hero h1"))
      .find(el => /wellness/i.test(el.textContent || ""));
    if(hero){
      hero.innerHTML = "Find Your Wellness<br>Direction.";
    }
  }

  function randomizeTerpeneExamples(){
    document.querySelectorAll(
      "#terpeneExplorer .tags, #terpeneExplorer .similar-scroll, .terpene-explorer .tags"
    ).forEach(container=>{
      const items = Array.from(container.children);
      if(items.length > 2){
        shuffle(items).forEach(el=>container.appendChild(el));
      }
    });

    // Randomize actual terpene cards too
    const terpeneWrap = document.querySelector("#terpeneExplorer") || document.querySelector(".terpene-explorer");
    if(terpeneWrap){
      const cards = Array.from(terpeneWrap.children).filter(el => (el.textContent||"").trim().length > 10);
      if(cards.length > 2){
        shuffle(cards).forEach(el=>terpeneWrap.appendChild(el));
      }
    }
  }

  function fixJournalPills(){
    document.querySelectorAll(".journal-feelings,.feelings-row,.quick-feelings,.mood-pills").forEach(row=>{
  ...
  document.addEventListener("DOMContentLoaded", ()=>{
    runV43();
    setTimeout(runV43, 300);
    setTimeout(runV43, 1000);
  });

  window.addEventListener("pageshow", ()=>setTimeout(runV43, 200));

})();



/* ===== V44: TRUE TERPENE RANDOM SAMPLING + JOURNAL CHIP UNCROP ===== */
(function(){
  const ALL_STRAIN_NAMES_FALLBACK = [
    "Blue Dream","Granddaddy Purple","Harlequin","ACDC","Cannatonic","Northern Lights",
    "Sour Diesel","Girl Scout Cookies","Pineapple Express","Wedding Cake","Gelato","Gorilla Glue #4",
    "Jack Herer","Green Crack","Durban Poison","Bubba Kush","OG Kush","Purple Punch",
    "Zkittlez","Runtz","Apple Fritter","Lemon Tree","Blue Cheese","Alien OG","Orange Cookies",
    "Platinum Cookies","Georgia Pie","Ghost Train Haze","Rainbow Belts","Pennywise","Jillybean",
    "Permanent Marker","Super Silver Haze","Purple Urkle","Banana OG","Animal Cookies",
    "Afghan Kush","Amnesia Haze","Acapulco Gold","Maui Wowie","Do-Si-Dos","Ice Cream Cake",
    "Cereal Milk","Cherry Pie","White Widow","Tangie","Forbidden Fruit","LA Confidential",
    "Chemdawg","Headband","Skywalker OG","Trainwreck","Blueberry","Strawberry Cough",
    "Lemon Haze","Mimosa","Sherbet","Papaya","Bruce Banner","GMO Cookies","Kush Mints",
    "Sunset Sherbet","Slurricane","MAC","Oreoz","Tropicana Cookies","Blackberry Kush"
  ];

  function getAllStrainNames(){
    const pools = [];

    // Try common global app data first.
    ["STRAINS","strains","strainData","STRAIN_DATA","strainDatabase","strainDB","allStrains"].forEach(key=>{
      if(Array.isArray(window[key])) pools.push(window[key]);
    });
    ...
    pools.flat().forEach(item=>{
      if(typeof item === "string") names.push(item);
        ...
        const n = item.name || item.title || item.strain;
        if(n) names.push(String(n));
      }
    });

    // Pull visible app names too, but not just alphabetized terpene chips.
    document.querySelectorAll("[data-strain], .strain-name, .strain-title, .card-title").forEach(el=>{
      const t=(el.textContent||"").trim();
      if(t && t.length > 2 && t.length < 35) names.push(t);
    });

    names = names.concat(ALL_STRAIN_NAMES_FALLBACK);
    return [...new Set(names)]
      .filter(n => !/^(mood|sleep|stress|body comfort|focus|cbd|thc|daytime|evening|balanced)$/i.test(n))
      .filter(n => n.length > 2);
  }

  function shuffle(a){
    const arr=[...a];
    // crypto-ish random when available
    for(let i=arr.length-1;i>0;i--){
      let r = Math.random();
      try {
  ...
  function pickRandomNames(count, offset){
    const all = shuffle(getAllStrainNames());
    const shifted = all.slice(offset).concat(all.slice(0, offset));
  ...
  function makeChip(name){
    ...
    chip.type = "button";
    chip.className = "tag strain-pill v44-random-chip";
  ...
  function trueRandomizeTerpeneExamples(){
    const terpeneCards = Array.from(document.querySelectorAll(
      "#terpeneExplorer .terpene-card, .terpene-explorer .terpene-card, #learn .terpene-card, #terpeneExplorer > article, #terpeneExplorer > div"
    )).filter(card => {
      const txt = (card.textContent || "").toLowerCase();
      return /myrcene|limonene|pinene|linalool|caryophyllene|terpinolene|humulene|ocimene/.test(txt);
    });

    terpeneCards.forEach((card, idx)=>{
      let row = card.querySelector(".tags,.strain-tags,.example-strains,.similar-scroll");
      if(!row){
        row = document.createElement("div");
        row.className = "tags v44-random-terpene-row";
        card.appendChild(row);
      }

      // Replace existing early-alphabet examples with real random examples.
      row.innerHTML = "";
      pickRandomNames(4, idx * 9 + Math.floor(Math.random()*7)).forEach(name => row.appendChild(makeChip(name)));
    });
  }

  function fixJournalChips(){
    const saved = document.querySelector("#saved");
    if(!saved) return;

    const rows = Array.from(saved.querySelectorAll(".journal-feelings,.feelings-row,.quick-feelings,.mood-pills,.chip-row,.chips,.option-pills"));
    rows.forEach(row=>{
      row.style.display = "flex";
      row.style.flexWrap = "wrap";
      row.style.overflow = "visible";
      ...
      row.style.gap = "10px";
      row.querySelectorAll("button,.feeling-pill,.mood-pill,*").forEach(el=>{
        if(el.children.length === 0 && (el.textContent || "").trim().length){
          ...
          el.style.padding = el.tagName === "BUTTON" ? "12px 16px" : el.style.padding;
        }
      });
    });

    // Screenshot shows this row may be a direct row between selects and textarea.
    const journal = saved.querySelector(".journal-card,.wellness-journal,.journal-form,[class*='journal']");
    if(journal){
      Array.from(journal.children).forEach(child=>{
        const hasManyButtons = child.querySelectorAll && child.querySelectorAll("button").length >= 3;
        if(hasManyButtons){
  ...
  function fixHeroV44(){
    const hero = Array.from(document.querySelectorAll("#home h1,.hero h1,.home-hero h1"))
      .find(el => /find\s+your\s+wellness/i.test(el.textContent || ""));
    if(hero){
      hero.innerHTML = "Find Your Wellness<br>Direction.";
      hero.style.textTransform = "none";
      hero.style.textAlign = "center";
    }
  }
  ...
  function runV44(){
    fixHeroV44();
    fixJournalChips();
    trueRandomizeTerpeneExamples();
  }

  document.addEventListener("DOMContentLoaded", ()=>{
    runV44();
    setTimeout(runV44, 250);
    setTimeout(runV44, 900);
    setTimeout(runV44, 1800);
  });

  window.addEventListener("pageshow", ()=>setTimeout(runV44, 200));
  window.addEventListener("hashchange", ()=>setTimeout(runV44, 150));
  document.addEventListener("click", ()=>setTimeout(runV44, 120), true);

  const oldShowPage = window.showPage;
  if(typeof oldShowPage === "function"){
    window.showPage = function(){
      const result = oldShowPage.apply(this, arguments);
      setTimeout(runV44, 180);
      setTimeout(runV44, 700);
      return result;
    };
  }
})();
