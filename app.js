const APP_VERSION="v104-clean-rebuild";
...
/* ======================================================
   ...
   - Restores normal scrolling and stable bottom navigation.
====================================================== */
(function(){
  const VERSION = "v104-clean-rebuild";
  const $id = (id)=>document.getElementById(id);
  const pageIds = ["home","search","recommend","saved","learn"];
  const safe = (name)=>String(name||"").replace(/'/g,"\\'");
  const originalRenderEducation = typeof window.renderEducation === "function" ? window.renderEducation : null;
  const originalRenderSaved = typeof window.renderSaved === "function" ? window.renderSaved : null;
  const originalRenderJournal = typeof window.renderJournal === "function" ? window.renderJournal : null;
  const originalLoadJournalSelect = typeof window.loadJournalSelect === "function" ? window.loadJournalSelect : null;
  const originalDailyTip = typeof window.dailyTip === "function" ? window.dailyTip : null;
  const originalUpdateStats = typeof window.updateStats === "function" ? window.updateStats : null;
  const originalRenderFilters = typeof window.renderFilters === "function" ? window.renderFilters : null;
  const unique = (arr)=>[...new Set((arr||[]).filter(Boolean))];
  const byName = (a,b)=>String(a?.name||a?.[0]||"").localeCompare(String(b?.name||b?.[0]||""));
  const strainByName = (name)=> (typeof getStrain === "function" ? getStrain(name) : (strains||[]).find(s=>s.name===name));
  const imgFor = (s)=> (typeof strainImage === "function" ? strainImage(s) : (s?.image||""));
  const textFor = (s)=> {
    if(typeof searchableText === "function") return searchableText(s);
    return [s.name,s.type,s.category,s.thc,s.cbd,s.time,s.flavor,s.insight,(s.tags||[]).join(" "),(s.goals||[]).join(" "),(s.terpenes||[]).join(" ")].join(" ").toLowerCase();
  };

  function setVersion(){
    document.documentElement.classList.add(VERSION);
    document.body.classList.remove(
      "v29-structural-polish","v30-mobile-layout-cleanup","v36-app-store-polish","v38-immersion","v39-polish","v41-force-visible",
      "v65-real-motion-nav-lock","v66-density-smart-layout","v67-learn-search-fix","v68-smart-density-repair","v69-final-spacing-learn-polish",
  ...
  function cleanSplash(){
    ...
    if(splash){ splash.style.opacity="0"; setTimeout(()=>splash.remove(),260); }
  }
  ...
  function cardHTMLV105(s,opts={}){
    ...
    return `<article class="strain-card v104-strain-card ${opts.home?'v104-home-card':''} ${opts.compact?'v104-compact-card':''}" onclick="openModal('${safe(s.name)}')">
      <div class="card-hero"><img src="${imgFor(s)}" alt="${s.name} cannabis strain image" loading="lazy" onerror="this.closest('.card-hero')?.classList.add('img-fallback')"></div>
      <div class="card-topline"><span class="eyebrow">${s.type||s.category||'Strain direction'}</span><b>${fit}% fit</b></div>
      <h3>${s.name}</h3>
      <p>${summary}</p>
  ...
  function renderSearchV105(){
    ...
    const filtered=[...strains].sort(byName).filter(s=>{
  ...
  function renderRecentHomeV105(){
    ...
    if(!recent.length){
      box.innerHTML = `<div class="empty-state">Open a strain profile and it will appear here.</div>`;
      return;
    }
    box.innerHTML = `<div class="v104-recent-grid">${recent.map(s=>`<button type="button" class="v104-recent-card" onclick="openModal('${safe(s.name)}')">
      <span class="v104-thumb"><img src="${imgFor(s)}" alt="" loading="lazy"></span><strong>${s.name}</strong><small>${s.category||s.time||'Direction'}</small>
    </button>`).join("")}</div>`;
  }
  ...
  function renderSmartInsightsV105(){
    ...
    const pick = [...strains].sort((a,b)=>(b.score||0)-(a.score||0))[0];
    box.innerHTML = `<div class="v104-smart-card">
      <span class="eyebrow">Smart Recommendations</span>
      <h2>Compare your next direction.</h2>
      <p>Use strain profiles, terpene notes, THC sensitivity, saved favorites, and journal history to compare educational wellness directions.</p>
      <div class="v104-action-row">
        <button type="button" onclick="showPage('recommend')">Start Match</button>
        <button type="button" class="ghost" onclick="showPage('search')">Browse Strains</button>
      </div>
      <div class="v104-mini-metrics"><span><b>${strains.length}</b> strains</span><span><b>${savedCount}</b> saved</span><span><b>${journalCount}</b> notes</span></div>
      ${pick?`<button type="button" class="v104-soft-link" onclick="openModal('${safe(pick.name)}')">Today’s spotlight: ${pick.name}</button>`:""}
    </div>`;
  }
  ...
  function renderTerpeneExplorerV105(){
    ...
    explorer.innerHTML = `<span class="eyebrow">Terpene Explorer</span><h3>Compare aroma profiles.</h3><p>Terpenes are aromatic compounds often associated with different cannabis aromas and user-reported directions. Effects vary by product and person.</p>
      <div class="v104-terp-grid">${list.map(([name,desc],idx)=>{
        let examples=[...strains].filter(s=>(s.terpenes||[]).includes(name));
        examples = examples.sort((a,b)=>((a.name.length+idx)%7)-((b.name.length+idx)%7) || a.name.localeCompare(b.name)).slice(0,4);
        return `<article class="terpene-card v104-terp-card"><h4>${name}</h4><p>${desc}</p><div class="learn-example-row">${examples.map(s=>`<button type="button" onclick="openModal('${safe(s.name)}')">${s.name}</button>`).join("") || `<span class="tag">Compare products by lab profile</span>`}</div></article>`;
      }).join("")}</div>`;
  }
  ...
  function removeJournalDuplicateChips(){
    const chips=$id("journalMoodChips");
    if(chips){
      chips.innerHTML = "";
  ...
  function fixBottomNav(){
    ...
    nav.querySelectorAll("button").forEach(btn=>{ btn.style.pointerEvents="auto"; btn.disabled=false; });
  }

  function enhanceHorizontalRow(el){
    if(!el) return;
    el.classList.add("v104-scroll-row");
  ...
  function normalizeScroll(){
    document.documentElement.classList.remove("sr-modal-open");
    document.body.classList.remove("sr-modal-open");
    if($id("strainModal") && !$id("strainModal").classList.contains("hidden")){
      document.documentElement.classList.add("sr-modal-open");
      document.body.classList.add("sr-modal-open");
    } else {
      document.documentElement.style.overflow="";
  ...
  function showPageV105(id){
    ...
    if(id==="saved"){
      ...
      if(typeof originalRenderSaved === "function") originalRenderSaved();
  ...
  // Run immediately because this script is at the end of body.
  run();
})();
