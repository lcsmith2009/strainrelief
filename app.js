const APP_VERSION="v90-home-layout-overlap-fix";
...
/* ==================================================
   V90 HOME LAYOUT OVERLAP FIX
   Home-only stabilization: removes old stacked CTA output, locks Home load
   layout, restores clean spacing, and uses native horizontal carousels.
================================================== */
(function(){
  const VERSION='v90-home-layout-overlap-fix';
  const $=(id)=>document.getElementById(id);
  ...
  function cleanHomeMotion(){
    document.body.classList.add(VERSION);
    qsa('#home .hero,#home .daily-card,#home #v27Spotlight,#home .quick-row,#home .pro-card,#home #livelyDashboard,#home #smartInsights,#home #categoryRibbons,#home .section-title,#home #trendingGrid,#home #recentHome').forEach(el=>{
      el.classList.remove('reveal-on-scroll','revealed','visible','v27-rise','sr-motion-ready','sr-motion-in','is-dragging');
  ...
  function cleanSmart(){
    const box=$('smartInsights');
    if(!box) return;
    qsa('.v83-smart-action,.v84-smart-action,.v85-smart-action,.v86-smart-action,.v87-smart-action,.v88-smart-action,.v89-smart-action',box).forEach(el=>el.remove());
    qsa('.panel-head button,.panel-head .small-btn',box).forEach(el=>el.remove());
  ...
  function centerLiveDashboard(){
    const board=$('livelyDashboard');
    if(!board) return;
    const strip=qs('.live-hero-strip',board);
    if(!strip) return;
    strip.style.display='grid';
    strip.style.gridTemplateColumns='1fr';
    strip.style.justifyItems='center';
    strip.style.textAlign='center';
    qsa('img',strip).forEach(img=>{img.style.margin='0 auto 8px'; img.style.display='block';});
    qsa('h2,h3,p,span',strip).forEach(el=>{el.style.textAlign='center';});
  }

  function normalizeTrending(){
    const row=$('trendingGrid');
    if(!row) return;
    row.classList.add('v90-native-carousel');
    ...
    row.style.gap='14px';
    row.style.padding='8px 22px 24px 8px';
    qsa('.strain-card',row).forEach(card=>{
      card.style.flex='0 0 82vw';
      card.style.width='82vw';
      card.style.minWidth='82vw';
      card.style.maxWidth='82vw';
  ...
  function normalizeRecent(){
    const box=$('recentHome');
    if(!box) return;
    const row=qs('.recent-carousel,.recent-mini-grid',box);
    if(!row) return;
    row.classList.add('v90-recent-carousel');
    row.style.display='flex';
    row.style.flexWrap='nowrap';
    row.style.overflowX='auto';
    row.style.overflowY='hidden';
    row.style.webkitOverflowScrolling='touch';
    row.style.gap='14px';
    row.style.padding='8px 24px 28px 8px';
    qsa('.recent-card,.recent-mini-card',row).forEach(card=>{
  ...
  if(typeof oldRenderFeatured==='function'){
    window.renderFeatured=function(){
      ...
      setTimeout(normalizeTrending,40);
      return result;
    };
  }
  window.StrainReliefV90HomeFix={version:VERSION,apply};
})();
